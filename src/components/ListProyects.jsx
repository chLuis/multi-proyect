export default function ListProyects() {
    ...
    return (
        ...
        <ul className="w-full animate-width grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 text-ellipsis text-nowrap px-4 lg:px-0 duration-200">
            {LIST_PROYECTS.map(({name, link, description, image, techs}, index) => 
                <a key={index} href={`/${link}`} className="relative flex flex-col rounded-lg font-sans overflow-clip h-[340px] group bg-white hover:bg-gray-100 hover:scale-[1.02] border border-gray-500 hover:shadow hover:shadow-gray-700 duration-200">
                    ...
                    <div className="flex gap-1">
                      {techs.map((tech, index) => 
                        <span key={index} className="bg-gray-200 uppercase text-gray-900 font-semibold text-xs hover:bg-blue-950 hover:text-white duration-200 px-2 py-1 rounded-full">
                          {tech}
                        </span>)}
                    </div>
