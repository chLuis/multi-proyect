    ...
    return (
        <div className="mt-10 flex flex-col mx-auto gap-4 min-w-0 w-fit max-w-[680px]">
            {data?.sort((a, b) => a.completed - b.completed || b.id - a.id).map((item, index) => (
                    <label name={index+"todo"} key={index} className={`flex ${item.completed ? "bg-neutral-700" : ""} items-center w-fit px-4 py-2 gap-4 min-w-0 max-w-[500px] border hover:shadow-none shadow-white shadow-sm hover:shadow-neutral-200 duration-200`}>
                        {item.completed 
