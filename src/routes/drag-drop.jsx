...
export default function DragAndDropPage() {
    ...
    return (
        ...
        <div onDragOver={handleOverAll} className="mx-4 animate-pulse-veryshort">
            <header className="flex flex-col justify-center items-center mt-10 mb-10">
                ...
                <a href="/" className="text-center font-semibold border-b-2 border-b-transparent px-4 xs:px-20 hover:border-b-white hover:px-0 duration-200">
                    Back to home
                </a>
            </header>
            ...
            <section className="flex flex-col md:flex-row gap-2 justify-center w-full text-center select-none">
                <div>
                <div onDragOver={(e) => e.stopPropagation()} onDrop={(e) => e.stopPropagation()} className="w-full min-h-60 border rounded bg-blue-600">
                    <h4 className="min-h-6">Zona de landing</h4>

                    <div onDrop={handleOnDrop} onDragOver={handleDragOver} className={`min-w-full min-h-60 z-10 ${hoverDrag}`}>
                        {landingZone.map((item, index) => (
                            <option draggable onDragStart={(e) => handleOnDragList(e)} onDragEnd={handleEndAll} key={index} value={index} className="hover:bg-slate-900 duration-200">
                    ...
                    <div className="min-h-14 flex justify-center items-center w-full">
                        {showTrash &&<div onDragOver={handleOverTrash} onDrop={handleTrash} className={`${hoverTrash} min-h-14 w-full flex justify-center items-center rounded duration-200 animate-width`}>
                ...
                <div className="w-full">
                    ...
                    <div className="flex flex-row flex-wrap gap-1 justify-center items-center hover:bg-neutral-800 p-4">
                        {SET_ELEMENT.map((item, index) => (
                            <option value={item.price} key={index} draggable onDragStart={(e) => handleOnDrag(e)} onDragEnd={handleOverAll} className="w-28 border rounded p-2 hover:bg-slate-400 cursor-grab active:cursor-grabbing select-none duration-200">{item.name}</option>
                        ))}
                    </div>
                </div>
            </section>
